...
/**
 ...
 Der Schaden wird je nach Levelunterschied abgeschwŠcht oder verstŠrkt.
 Unterschied Maximal +- 10 Level und zwischen.
 Der "Gegner" ist immer der Angreifer.
 -10 Level: Schaden 0%
 0 Level: Schaden 100%
 +10 Level: Schaden 125%
 Wenn der Berechnete Schaden 0 ist, wird pauschal immer 1 Schaden zurŸckgegeben.
 */
int Character::calcDamage(int damage,int level){
	int levelDif = level-this->Lvl;
    float multi = 1.f;
    if(levelDif > 0){   // Gegner hat einen hšhreren Level
...
/**
 Diese Methode ist fŸr die Zahlen, die Ÿber die Charaktere hinweg schweben.
...
void Character::damageText(int damage, char type){
	...
	if( type == 'e') dmgString << "XP +";           // zusatztext fŸr exp und levelup
...
/**
 FŸr das Darstellen eines Charakters.
 Inaktive ("getštete") Charaktere sollen nicht neu gezeichnet werden.
 der dmgText wird in einem Vector gespeichert und soll auch angezeigt werden kšnnen,
